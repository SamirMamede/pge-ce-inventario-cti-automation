...
describe('US03: Geração de Termos de Atribuição', () => {
    ...
    it.only('Cenário 04: Gerar Termo de Empréstimo com Sucesso', () => {
        ...
        cy.get('@aberturaNovaAba').should('be.calledWithMatch', /term_responsibility_asset.*term_type=loan/);
    });

    it('Cenário 05: Validar se o PDF gerado contém todos os campos obrigatórios', () => {
                ...
                cy.then(() => {
                    ...
                    cy.request({
                        ...
                        cy.task('getPdfText', caminhoDoPdf).then((textoPDF) => {
                            ...
                            const regexLocalData = new RegExp(`(${cidadesRegex}),\\s*${dia}\\s*de\\s*${mesFormatado}\\s*de\\s*${ano}`, 'i');
                            ...
                            if (textoLimpo.match(/RESPONSABILIDADE/i)) {
                                expect(textoLimpo).to.include('declaro receber os bens relacionados no presente termo, no estado de conservação indicado, pelo qual assumo total responsabilidade pela guarda');
                            } else {
                                expect(textoLimpo).to.include('declaro receber na condição de empréstimo, os equipamentos listados pertencente à Procuradoria-Geral do Estado do Ceará');
                            }
                        });
                    });
                });
