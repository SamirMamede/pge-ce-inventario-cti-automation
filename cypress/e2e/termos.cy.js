...
describe('US03: Geração de Termos de Atribuição', () => {
  ...
  beforeEach(() => {
    ...
    it('Cenário 01: Validar comportamento de exclusão mútua dos checkboxes no modal', () => {
        ...
        AtribuicaoPage.chkResponsabilidade
        .check()
        .should('be.checked');

        AtribuicaoPage.chkEmprestimo
        .check()
        .should('be.checked');
    ...
    it('Cenário 02: Validar fechamento do modal pelo botão de fechar (X)', () => {
        ...
        AtribuicaoPage.btnFecharModal
            .invoke('click');

        AtribuicaoPage.modalTermos
  ...
  it('Cenário 05: Validar se o PDF contém todos os campos obrigatórios', () => {
    ...
    const caminhoDoPdf =
      'cypress/downloads/termo_validacao.pdf';
    ...
    cy.fixture('cidades').then((cidades) => {
      ...
      cy.then(() => {
        ...
        cy.request({
          ...
          cy.task('getPdfText', caminhoDoPdf)
            .then((textoPDF) => {

              const textoLimpo =
                textoPDF.replace(/\s+/g, ' ');
              ...
              const mesFormatado =
                mesPorExtenso.charAt(0).toUpperCase() +
                mesPorExtenso.slice(1);

              const cidadesRegex =
                cidades.join('|');

              const regexLocalData =
                new RegExp(
              ...
              expect(textoLimpo)
                .to.match(/ATIVOS ATRIBUIDOS/i);

              expect(textoLimpo)
                .to.match(/ASSINATURA/i);

              expect(textoLimpo)
                .to.match(regexLocalData);
