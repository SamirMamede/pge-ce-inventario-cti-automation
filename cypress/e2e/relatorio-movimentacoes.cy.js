    ...
    cy.request({
      ...
      cy.task("getPdfText", caminhoDoPdf).then((textoPDF) => {
        ...
        expect(textoLimpo).to.include(
          `Movimentações de Ativos para ${areaSelecionada} no período: 01/05/2026 à 17/05/2026`,
        );
        ...
        expect(textoLimpo).to.match(
          /15\s+de\s+Maio\s+de\s+2026\s*-\s*12\s+movimentaç/i,
        );
      });
    });
  });
